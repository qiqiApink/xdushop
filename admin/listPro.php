...
$sql = "select p.id, p.pName, p.pSn, p.pNum, p.mPrice, p.xduPrice, p.pDesc, p.pubTime, p.isShow, p.isHot, c.cName from xdushop_pro as p join xdushop_cate c on p.cId = c.id {$where}";
...
$sql = "select p.id, p.pName, p.pSn, p.pNum, p.mPrice, p.xduPrice, p.pDesc, p.pubTime, p.isShow, p.isHot, c.cName from xdushop_pro as p join xdushop_cate c on p.cId = c.id {$where} {$orderBy} limit {$offset},{$pageSize}";
$rows = fetchAll($link, $sql);
if(!$rows)
{
    alertMes('Sorry, there is no product. Please add it first.', 'addPro.php');
    exit;
}
?>
<!doctype html>
...
<div class="details">
            ...
            <div class="text">
                ...
                <div class="bui_select">
                    <select id="" class="select" onchange="change(this.value)">
                        <option>-请选择-</option>
                        <option value="xduPrice asc" >由低到高</option>
                        <option value="xduPrice desc">由高到底</option>
                    </select>
                </div>
            </div>
            <div class="text">
                    ...
                    <td align="center">
                        ...
                        <div id="showDetail<?php echo $row['id'];?>" style="display:none;">
                            <table class="table" cellspacing="0" cellpadding="0">
                                ...
                                <tr>
                                    <td  width="20%"  align="right">学习商城价格</td>
                                    <td><?php echo $row['xduPrice'];?></td>
                                </tr>
                                <tr>
    ...
    function delPro(id){
        if(window.confirm("您确认要删除嘛？删除之后不可以恢复哦！！！")){
